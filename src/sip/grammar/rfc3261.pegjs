 ...
 */
// Object to which header attributes will be assigned during parsing
{ var data = {}; var global = goog.global; }
...
SIP_URI_noparams  = uri_scheme ":"  userinfo ? hostport {
                    try {
                        data.uri = new jssip.uri.Uri.Builder().
                            addPropertyPair(jssip.uri.Uri.PropertyName.SCHEME, data.scheme).
                            addPropertyPair(jssip.uri.Uri.PropertyName.USER, data.user).
                            addPropertyPair(jssip.uri.Uri.PropertyName.HOST, data.host).
                            addPropertyPair(jssip.uri.Uri.PropertyName.PORT, data.port).build();
                        delete data.scheme;
                        delete data.user;
                        delete data.host;
...
SIP_URI         = uri_scheme ":"  userinfo ? hostport uri_parameters headers ? {
                    ...
                    try {
                        data.uri = new jssip.uri.Uri.Builder().
                            ...
                            addPropertyPair(jssip.uri.Uri.PropertyName.USER, data.user).
                            addPropertyPair(jssip.uri.Uri.PropertyName.HOST, data.host).
                            addPropertyPair(jssip.uri.Uri.PropertyName.PORT, data.port).
                            addPropertyPair(jssip.uri.Uri.PropertyName.PARAMETERS, data.uri_params).
                            addPropertyPair(jssip.uri.Uri.PropertyName.HEADERS, data.uri_headers).build();
                        delete data.scheme;
                        delete data.user;
                        delete data.host;
                        delete data.host_type;
...
userinfo        = user (":" password)? "@" {
                    data.user = global.decodeURIComponent(input.substring(pos-1, offset));}
...
contact_param       = (addr_spec / name_addr) (SEMI contact_params)* {
                        ...
                        try {
                          header = new jssip.sip.protocol.NameAddr(data.uri, data.display_name, data.params);
                          delete data.uri;
...
From        = ( addr_spec / name_addr ) ( SEMI from_param )* {
                var tag = data.tag;
                try {
                  data = new jssip.sip.protocol.NameAddr(data.uri, data.display_name, data.params);
